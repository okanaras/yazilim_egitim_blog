...
@section('title')
    Makale {{ isset($category) ? "Guncelleme" : "Ekleme" }}
@endsection
...
@section('content')
    <x-bootstrap.card>
        <x-slot:header>
            <h2 class="card-title">    Makale {{ isset($category) ? "Guncelleme" : "Ekleme" }}
            </h2>
        </x-slot:header>
        <x-slot:body>
            ...
            <div class="example-container">
                <div class="example-content">
                    ...
                    <form action="{{ isset ($category) ?  route('categories.edit', ['id'=> $category->id]) : route('category.create') }}" method="POST">
                        ...
                        >   
                        <select 
                            class="form-select bg-light m-b-sm" 
                            aria-label="Ust Kategori Secimi"
                            name="parent_id"
                        >
                        ...
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" value="1" name="status" id="status" {{ isset($category) && $category->status ? "checked" : "" }}>
                            <label for="status" class="form-check-label">
                                Kategori Sitede Gorunsun mu?
                            </label>    
                        </div>
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" value="1" name="feature_status" id="feature_status" {{ isset($category) && $category->feature_status ? "checked" : "" }}>
                        ...
                        <div class="col-6 mx-auto mt-5">
                            <button type="submit" class="btn btn-success btn-rounded w-100">{{ isset($category) ? "Guncelle" : "Kaydet" }}
                            </button>
                        </div>
                    </form>
                </div>
